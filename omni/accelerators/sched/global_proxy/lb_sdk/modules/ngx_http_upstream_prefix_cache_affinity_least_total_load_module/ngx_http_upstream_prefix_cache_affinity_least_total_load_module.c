...
} ngx_http_prefix_cache_affinity_least_total_load_conf_t;
...
static void 
*ngx_http_prefix_cache_affinity_least_total_load_create_srv_conf(ngx_conf_t *cf)
{
...
static ngx_int_t ngx_http_least_total_load_init_shm_zone(
    ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    ngx_log_error(NGX_LOG_WARN, shm_zone->shm.log, 0,
                  "[least_total_load:init_shm_zone] shm addr=%p, data=%p",
                  shm_zone->shm.addr, data);
    ...
    shm_block->peer_count = MAX_PEERS;
    ...
    ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    cache = ngx_slab_alloc(shpool, sizeof(ngx_http_dictionary_t));
    if (cache == NULL) {
        return NGX_ERROR;
    }
...
ngx_http_upstream_prefix_cache_affinity_least_total_load_commands[] = {
    {ngx_string("prefix_cache_affinity_least_total_load"),
     NGX_HTTP_UPS_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot,
     NGX_HTTP_SRV_CONF_OFFSET,
     offsetof(ngx_http_prefix_cache_affinity_least_total_load_conf_t, enable),
     NULL},
    {ngx_string("least_total_load_batch_size"),
     NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot,
     NGX_HTTP_SRV_CONF_OFFSET,
     offsetof(ngx_http_prefix_cache_affinity_least_total_load_conf_t, batch_size),
     NULL},
    {ngx_string("prefix_cache_affinity_dict_size"),
     NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot,
     NGX_HTTP_SRV_CONF_OFFSET,
     offsetof(ngx_http_prefix_cache_affinity_least_total_load_conf_t, cache_size),
     NULL},
     {ngx_string("prefix_cache_affinity_block_size"),
        NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
        ngx_conf_set_num_slot,
        NGX_HTTP_SRV_CONF_OFFSET,
        offsetof(ngx_http_prefix_cache_affinity_least_total_load_conf_t, block_size),
        NULL},
     ngx_null_command};

static ngx_http_module_t 
ngx_http_upstream_prefix_cache_affinity_least_total_load_module_ctx = {
    NULL,                                        /* preconfiguration */
    ngx_http_prefix_cache_affinity_least_total_load_postconfig,
    NULL,                                        /* create_main_conf */
    NULL,                                        /* init_main_conf */
    ngx_http_prefix_cache_affinity_least_total_load_create_srv_conf,
    NULL,                                        /* merge_srv_conf */
    NULL,                                        /* create_loc_conf */
    NULL};                                       /* merge_loc_conf */

ngx_module_t ngx_http_upstream_prefix_cache_affinity_least_total_load_module = {
    NGX_MODULE_V1,
    &ngx_http_upstream_prefix_cache_affinity_least_total_load_module_ctx,
    ngx_http_upstream_prefix_cache_affinity_least_total_load_commands,
    NGX_HTTP_MODULE,
...
static ngx_int_t 
prefix_cache_affinity_least_total_load_select(
    ...
    ngx_uint_t peer_count)
{
    ...
    for (ngx_uint_t i = 0; i < peer_count; ++i) {
        ngx_uint_t length_sum_peers = ngx_atomic_fetch_add(
            &(prefill_shm->peers[i].total_length_sum), 0);
        ngx_uint_t request_sum_peers = ngx_atomic_fetch_add(
            &(prefill_shm->peers[i].total_request_sum), 0);
    ...
    ngx_shmtx_lock(&least_total_load_shpool->mutex);
    ngx_atomic_fetch_add(
        &(prefill_shm->peers[chosen].total_length_sum), 
        (ngx_atomic_int_t)request_length);
    ngx_atomic_fetch_add(
        &(prefill_shm->peers[chosen].total_request_sum), 
        (ngx_atomic_int_t)1);
    ngx_shmtx_unlock(&least_total_load_shpool->mutex);
    ...
    ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    ngx_http_upstream_rr_peer_data_t         *rrp;
    ngx_http_prefix_cache_affinity_least_total_load_peer_data_t   *pdata;
    ngx_uint_t                               chosen = 0;
    ngx_uint_t                               peer_count;
    ...
    peer_count = ngx_min(peer_count, prefill_shm->peer_count);
    if (peer_count == 0) {
        return NGX_ERROR;
    }
    
    chosen = prefix_cache_affinity_least_total_load_select(
        prefill_shm, r, (ngx_uint_t) r->request_length, peer_count);
    ...
    u->peer.data = pdata;
    u->peer.get  = ngx_http_prefix_cache_affinity_least_total_load_get_peer;
    u->peer.free = ngx_http_prefix_cache_affinity_least_total_load_free_peer;
...
static ngx_int_t ngx_http_prefix_cache_affinity_least_total_load_get_peer(
    ngx_peer_connection_t *pc, void *data)
{
    ngx_http_prefix_cache_affinity_least_total_load_peer_data_t *pdata = data;
...
ngx_http_prefix_cache_affinity_least_total_load_postconfig(ngx_conf_t *cf)
{
    ...
    if (ngx_http_least_total_load_shm_size == 0) {
        ngx_http_least_total_load_shm_size = 32 * ngx_pagesize;
    }
    if (ngx_http_prefix_cache_affinity_shm_size == 0) {
        ngx_http_prefix_cache_affinity_shm_size = 2048 * ngx_pagesize;
    }

    least_total_load_shm_name.len  
        = sizeof("upstream_prefill_least_total_load")  - 1;
    least_total_load_shm_name.data 
        = (u_char *)"upstream_prefill_least_total_load";
    ...
    for (i = 0; i < upcf->upstreams.nelts; i++) {
        conf = ngx_http_conf_upstream_srv_conf(
            uscfp[i],
            ngx_http_upstream_prefix_cache_affinity_least_total_load_module);
        ...
        ngx_http_least_total_load_shm_zone 
            = ngx_shared_memory_add(
                cf,
                &least_total_load_shm_name,
                ngx_http_least_total_load_shm_size,
                &ngx_http_upstream_prefix_cache_affinity_least_total_load_module);
