...
typedef struct {
    ...
    ngx_uint_t active_request_lengths[MAX_ACTIVE_REQ_PER_PEER];
    void* active_request_ptrs[MAX_ACTIVE_REQ_PER_PEER];
...
static void* ngx_http_pd_score_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (conf == NULL) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0, "Failed to allocate pd_score_balance main conf");
        return NULL;
    }

    conf->shm_size = NGX_CONF_UNSET_SIZE;
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, cf->log, 0, "pd_score_balance main conf created");
    return conf;
...
static char* ngx_http_pd_score_set_mode(ngx_conf_t* cf, ngx_command_t* cmd, void* conf)
{
    ...
    if (ngx_strcmp(value[1].data, "prefill") == 0) {
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid pd_score_balance mode: %V", &value[1]);
...
static char* ngx_http_pd_score_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
...
{
    ngx_http_pd_score_peer_data_t *pdata = r->upstream ? r->upstream->peer.data : NULL;
    ngx_slab_pool_t *shpool;
    if (pd_shm == NULL || pdata == NULL) return;
    shpool = (ngx_slab_pool_t *)ngx_http_pd_score_shm_zone->shm.addr;
    ngx_shmtx_lock(&shpool->mutex);

    ngx_atomic_fetch_add(&pd_shm->peers_D[pdata->chosen].total_decode_num, (ngx_atomic_int_t)num_tokens);
    ngx_atomic_fetch_add(&pdata->decode_token_count, (ngx_atomic_int_t)num_tokens);

    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
        "[PDScore] peer=%ui request=%p decode_token +%ui, peer_total_decode_token=%uA, request_total_decode_token=%uA",
        pdata->chosen,
        r,
        num_tokens,
...
static ngx_int_t ngx_http_pd_score_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    // 获取 upstream 配置
    uscf = ngx_http_conf_upstream_srv_conf(r->upstream->upstream, ngx_http_upstream_pd_score_balance_module);
    if (uscf->mode == PD_MODE_PREFILL) {
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "prefill mode, ignore filter");
        return ngx_http_next_body_filter(r, in);
    }
    ngx_chain_t* cl;
...
static ngx_int_t ngx_http_pd_score_prefill_strategy(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    if (pd_shm->total_active_request_count < MAX_ACTIVE_REQ_PER_PEER) {
        ngx_uint_t idx = pd_shm->total_active_request_count++;
        pd_shm->active_request_lengths[idx] = (ngx_uint_t)r->request_length;
        pd_shm->active_request_ptrs[idx] = pdata;
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "[PDScore-PREFILL] request %p add to queue", pdata);
    }

    ngx_atomic_fetch_add(&peer->active_requests, 1);
    ngx_atomic_fetch_add(&peer->total_time_cost, (ngx_atomic_int_t)my_time_cost);
    ngx_shmtx_unlock(&shpool->mutex);
...
static ngx_int_t ngx_http_pd_score_decode_strategy(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    ngx_uint_t batch_req_lengths[MAX_TOTAL_ACTIVE_REQS];

    ngx_uint_t batch_count = pd_shm->total_active_request_count + 1;
    ngx_uint_t j = 0;
    for (j = 0; j < pd_shm->total_active_request_count; j++) {
        batch_req_lengths[j] = pd_shm->active_request_lengths[j];
    }
    batch_req_lengths[pd_shm->total_active_request_count] = (ngx_uint_t)r->request_length;
    qsort(batch_req_lengths, batch_count, sizeof(ngx_uint_t), cmp_desc);
    for (j = 0; j < batch_count; j++) {
    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "sorted batch_req_lengths[%ui]: %ui", j, batch_req_lengths[j]);
    }


    double peer_loads[MAX_PEER_COUNT];
    for (i = 0; i < n; i++) {
        peer_loads[i] = pd_shm->peers_D[i].total_decode_num * 4; // decode num -> length 转换
    }
    ...
    for (i = 0; i < batch_count;i++) {
        ...
        for (ngx_uint_t j = 1; j < n; j++) {
            if (peer_loads[j] < min_load && pd_shm->peers_D[j].active_requests <= N_REQ_LIM)
            {
        ...
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
            "[PDScore-DECODE-LPT] predict simul req.%ui dispatched to %ui", i, min_peer);
            if (batch_req_lengths[i] == (ngx_uint_t)r->request_length && chosen == 0) {
                chosen = min_peer;
                break;
            }
    }
...
static void ngx_http_pd_score_free_peer_P(ngx_peer_connection_t * pc, void *data, ngx_uint_t state)
{
    ...
    ngx_log_error(NGX_LOG_INFO, pc->log, 0, "total active request count: %ui", pd_shm->total_active_request_count);

    for (ngx_uint_t i = 0; i < pd_shm->total_active_request_count; i++) {
        if (pd_shm->active_request_ptrs[i] == (void *)pc->data)
        {
            ngx_log_error(NGX_LOG_INFO, pc->log, 0, "free P request %p, idx: %ui", pc->data, i);
            if (pd_shm->total_active_request_count > 1)
            {
                pd_shm->active_request_lengths[i] = pd_shm->active_request_lengths[pd_shm->total_active_request_count - 1];
                pd_shm->active_request_ptrs[i] = pd_shm->active_request_ptrs[pd_shm->total_active_request_count - 1];
            }
            pd_shm->total_active_request_count--;
            break;
        }
        ngx_atomic_fetch_add(&peer->active_requests, (ngx_atomic_int_t)-1);
        ngx_atomic_fetch_add(&peer->total_request_length, (ngx_atomic_int_t) - (pdata->request_length));
        ngx_atomic_fetch_add(&peer->total_time_cost, (ngx_atomic_int_t) - (pdata->my_time_cost));
        ngx_atomic_fetch_add(&peer->total_decode_num, (ngx_atomic_int_t) - (pdata->decode_token_count));
...
{
    ngx_http_pd_score_conf_t *conf = ngx_http_conf_upstream_srv_conf(uscf, ngx_http_upstream_pd_score_balance_module);
...
{
    ngx_http_pd_score_conf_t *pmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_upstream_pd_score_balance_module);
    ...
    if (pmcf->shm_size == 0 || pmcf->shm_size == NGX_CONF_UNSET_SIZE) {
        ...
        ngx_log_error(NGX_LOG_NOTICE, cf->log, 0, "Using default shm_size: %uz bytes", pmcf->shm_size);
    }
    ...
    ngx_http_pd_score_shm_zone = ngx_shared_memory_add(
        cf, shm_name, ngx_http_pd_score_shm_size, &ngx_http_upstream_pd_score_balance_module);
    ...
    for (i = 0; i < upcf->upstreams.nelts; i++) {
        conf = ngx_http_conf_upstream_srv_conf(uscfp[i], ngx_http_upstream_pd_score_balance_module);
        if (conf->mode != PD_MODE_NONE) {
            uscfp[i]->peer.init = ngx_http_pd_score_upstream_init;
        }
        ngx_log_error(NGX_LOG_WARN, cf->log, 0, "[PDScoreBalance] upstream[%ui] mode=%d", i, conf->mode);
    }
    return NGX_OK;
}
...
static ngx_command_t ngx_http_upstream_pd_score_commands[] = {
    {ngx_string("pd_score_balance"),
     ...
     ngx_http_pd_score_set_mode,
     NGX_HTTP_SRV_CONF_OFFSET,
     offsetof(ngx_http_pd_score_conf_t, mode),
     NULL},
    {ngx_string("pd_score_balance_shm_size"),
     NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot,
     NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_pd_score_conf_t, shm_size),
     NULL},
    ngx_null_command
