...
void ngx_http_weighted_least_active_add_decoded_tokens(ngx_http_request_t *r, ngx_uint_t num_tokens) {
    ...
    if (wla_shm == NULL || pdata == NULL) return;
    ...
    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
        "[WeightedLeastActive] peer=%ui request=%p decode_token +%ui, peer_total_decode_token=%uA, request_total_decode_token=%uA",
        pdata->chosen,
        r,
        num_tokens,
...
ngx_http_weighted_least_active_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = in; cl; cl = cl->next) {
        if (cl->buf->last > cl->buf->pos) {
            ...
            if (found) {
                ...
                while (num_start < last && (*num_start == ' ' || *num_start == '\"')) num_start++;
                ngx_uint_t val = 0;
    ...
    if (num_tokens > 0) {
        ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
            "[WLA-Filter] chunk tokens: %ui (prompt=%ui, output=%ui, first=%d)",
...
ngx_http_weighted_least_active_postconfig(ngx_conf_t *cf)
{
    ...
    shm_name->len = sizeof("weighted_least_active") - 1;
    shm_name->data = (u_char *)"weighted_least_active";
    ngx_http_weighted_least_active_shm_zone = ngx_shared_memory_add(
        cf, shm_name, ngx_http_weighted_least_active_shm_size, &ngx_http_upstream_weighted_least_active_module);
    ...
    for (i = 0; i < upcf->upstreams.nelts; i++) {
        ...
        // ngx_log_error(NGX_LOG_WARN, cf->log, 0, "[WLA] upstream[%ui] enable=%d", i, conf->enable);
    }
    return NGX_OK;
}
    ...
    ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    shpool = (ngx_slab_pool_t *)ngx_http_weighted_least_active_shm_zone->shm.addr;
    n = rrp->peers->number;
    if (n > wla_shm->peer_count) n = wla_shm->peer_count;
    ...
    // for (i = 0; i < n; i++) {
    //     ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
    //         "[WLA] Peer #%ui: active_requests=%uA, total_decode_num=%uA",
    //         i,
    //         wla_shm->peers[i].active_requests,
    //         wla_shm->peers[i].total_decode_num);
    // }
    ...
    ngx_atomic_fetch_add(&wla_shm->peers[chosen].total_request_length, (ngx_atomic_int_t)r->request_length);
    ...
    ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
        "[WLA] assign request(len=%ui) to peer #%ui", pdata->request_length, pdata->chosen);
...
ngx_http_weighted_least_active_free_peer(ngx_peer_connection_t *pc, void *data, ngx_uint_t state)
{
    ...
    ngx_atomic_fetch_add(&wla_shm->peers[pdata->chosen].total_request_length, (ngx_atomic_int_t)-(pdata->request_length));
