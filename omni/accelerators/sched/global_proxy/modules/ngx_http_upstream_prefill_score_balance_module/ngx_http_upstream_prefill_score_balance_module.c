...
static double ngx_http_prefill_score_time_cost(ngx_uint_t request_length) {
    double l = (double)request_length/4;
    return l * l * 0.0000009644 + l * 0.0500242701 + 2.0775952858;
}
...
static ngx_int_t
ngx_http_prefill_score_upstream_init(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *uscf)
{
    ngx_http_upstream_t* u = r->upstream;
    ngx_http_upstream_rr_peer_data_t* rrp;
    ngx_http_prefill_score_peer_data_t* pdata;
    ngx_uint_t chosen = 0, i, n;
    double min_load, load, my_time_cost;
    ngx_slab_pool_t* shpool;
    ...
    n = rrp->peers->number;
    if (n > pfs_shm->peer_count) n = pfs_shm->peer_count;
    ...
    ngx_atomic_fetch_add(&pfs_shm->peers[chosen].total_time_cost, my_time_cost);
...
ngx_http_prefill_score_postconfig(ngx_conf_t* cf)
{
    ...
    shm_name->len = sizeof("upstream_prefill_score") - 1;
    shm_name->data = (u_char*)"upstream_prefill_score";
    ngx_http_prefill_score_shm_zone = ngx_shared_memory_add(
        cf, shm_name, ngx_http_prefill_score_shm_size, &ngx_http_upstream_prefill_score_balance_module);
    ...
    for (i = 0; i < upcf->upstreams.nelts; i++) {
        ...
        // ngx_log_error(NGX_LOG_WARN, cf->log, 0, "[PrefillScoreBalance] upstream[%ui] enable=%d", i, conf->enable);
    }
    return NGX_OK;
}