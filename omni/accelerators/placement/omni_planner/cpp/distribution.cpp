...
Distribution::Distribution(size_t  rank, size_t world_size, const char* infoStr, HcclCommInitType type){
    ...
    ACLCHECK(aclrtMalloc(&data_ptr, world_size_*info_length_*sizeof(int), ACL_MEM_MALLOC_HUGE_FIRST));
    deviceHandshakeStatus_ = Tensor((uint64_t)data_ptr,world_size_*info_length_,sizeof(int),"int","constant tensor");
    deviceHandshakeStatus_.to_device(hostHandshakeStatus_.data());

    
    ACLCHECK(aclrtMalloc(&data_ptr, info_length_*sizeof(int), ACL_MEM_MALLOC_HUGE_FIRST));
    deviceCurrentStatus_ = Tensor((uint64_t)data_ptr,info_length_,sizeof(int),"int","constant tensor");
...
void* Distribution::get_recv_buff_address(){
    if (recv_buff_ == nullptr){
        throw std::runtime_error("Pls initilization recv_buff_ by allocate_recv_buffs");
    }
...
void Distribution::release_recv_buffs(){
    ACLCHECK(aclrtFree(recv_buff_));
}

void Distribution::enqueue(TransDesc* desc, size_t t_rank, bool need_enqueue_recv_buff){
    assert(desc != nullptr && "Adding an empty TransDesc ptr to the Queue");
    bool send_first = rank_<t_rank;
    desc->t_rank = t_rank;
    // TIME_IT_LABEL("Swap--rank_"+std::to_string(rank_),{
    TransDesc* position_recv_desc = completedSynchronizeQueue_.GetRear(desc->localExpertPositionOfsset);
    
    std::vector<void *> send_address = (position_recv_desc==nullptr)? desc->address: position_recv_desc->recv_buffs;
    // std::cout<<"rank["<<rank_<<"] send_address["<<send_address[0]<<"] recv_address["<<desc->recv_buffs[0]<<"]"<<std::endl;
    for (size_t idx=0; idx< desc->address.size();++idx){
        swap(send_address[idx], desc->recv_buffs[idx], desc->lengths[idx], desc->dtypes[idx], t_rank, send_first, stream_); // 对端队列满了， 等待超时交换失败
    }
    // });
    // assert( completedSynchronizeQueue_.IsFull() && "There is no Queue Space of completedSynchronizeQueue_ on func(Distribution::enqueue)");
    if (need_enqueue_recv_buff) completedSynchronizeQueue_.Enqueue(desc); 
}

// The global_position_id will be put a new expert with expert_id
bool Distribution::performGlobalHandshake(int t_rank, int position_offset, int expert_id){
    // world_size 为 队列满， -1为当前Rank已完成所有下发
    bool is_full = isCompletedQueueFull();
    if (is_full && (t_rank!=-1 && t_rank!= (int) rank_)) t_rank=(int) world_size_; // 队列已满，广播当前队列已满 (标志符)

    std::vector<int> hostCurrentStatus = {t_rank,position_offset,expert_id};

    deviceCurrentStatus_.to_device(hostCurrentStatus.data()); 
   
    allgather(deviceCurrentStatus_.get_data_ptr(),deviceHandshakeStatus_.get_data_ptr(), info_length_, "int");// 广播当前的目标队列
    deviceHandshakeStatus_.to_host(hostHandshakeStatus_.data());
    // if(rank_==0){
    //     std::string log= "handshake info \n ";
    //     for(size_t idx =0; idx<hostHandshakeStatus_.size();++idx){
    //         log += " "+std::to_string(hostHandshakeStatus_[idx]);
    //     }
    //     std::cout<<log<<std::endl;
    // }

    if (t_rank==(int)world_size_) {
        // std::cout<<"rank["<<rank_<<"] completed queue is full!"<<std::endl;
        return false; // 当前队列已满
    }
    
    else if (t_rank!=-1){
        //当前rank 还存在下发任务
        if (hostHandshakeStatus_[t_rank*info_length_]==(int)rank_) return true; // 对端跟自己握手
        // std::cout<<"rank["<<rank_<<"]'s t_rank["<<t_rank<<"] is want to handshake with rank["<< hostHandshakeStatus_[t_rank]<<"]"<<std::endl;
        return false; // 对端不跟自己握手
    }

    // 剩下 t_trank == -1 的情况
    for(size_t idx =0; idx<world_size_;++idx){
        if (hostHandshakeStatus_[idx*info_length_]!=-1) return false; // 还有rank存在swap 下发任务（队列满了等待状态），
    }
...
void Distribution::copyFromCompletedQueueToHBM() {
    ...
    for(size_t idx=0;idx<desc->recv_buffs.size();++idx){
        ACLCHECK(aclrtMemcpyAsync(desc->address[idx], desc->sizes[idx], desc->recv_buffs[idx], desc->sizes[idx], ACL_MEMCPY_DEVICE_TO_DEVICE,memcopy_stream_));
    }
...
void Distribution::allgather(void * src_addr, void * recv_addr,size_t length, std::string dtype){

    assert(stream_==nullptr && "stream_ should not be nullptr");
    HCCLCHECK(HcclAllGather(src_addr, recv_addr, length, NAME2DATATYPE.at(dtype), hcclComm_, stream_));
    ACLCHECK(aclrtSynchronizeStream(stream_));

}

void Distribution::set_stream(aclrtStream stream){
    stream_ = stream;
}
...
void Distribution::warmup()
{
    ...
    ACLCHECK(aclrtMalloc(&recv_buf, data_size*world_size_, ACL_MEM_MALLOC_HUGE_FIRST));

    if (rank_==0){
        HCCLCHECK(HcclSend(data_ptr, length, NAME2DATATYPE.at(dtype), 31, hcclComm_, stream));
    }
    else if(rank_==31){
        HCCLCHECK(HcclRecv(data_ptr, length, NAME2DATATYPE.at(dtype), 0, hcclComm_, stream));
    }

    ACLCHECK(aclrtSynchronizeStream(stream));

    HCCLCHECK(HcclAllGather(data_ptr, recv_buf, length, NAME2DATATYPE.at(dtype), hcclComm_, stream));
    ...
    if (rank_==0) std::cout<<"finished hcclcomm_ warmup!"<<std::endl;
}
