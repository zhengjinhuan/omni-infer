...
std::string getTimestamp() {
    ...
    // auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(since_epoch) % 1000; // 毫秒
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(since_epoch) % 1000000; // 微秒
    ...
    // 追加毫秒或微秒
    // oss << ":" << std::setfill('0') << std::setw(3) << ms.count() << "] "; // 毫秒版
    oss << ":" << std::setfill('0') << std::setw(6) << us.count() << "] ";   // 微秒版
...
struct TimeTracker {
    ...
    void check_and_print(const std::string& tag) {
        ...
        if(ms >= 1) {  // 仅打印超过1ms的耗时
            std::cout << getTimestamp() << "[elapse warn] " << tag << ": " << ms << "ms\n";
        }
...
void Placement::initialize_components(size_t expert_mapping_ptr, std::vector<int64_t> shape, int dtype,
    ...
    mapping_ = new PlacementMapping("", rank_, 1, num_devices_per_host_, placement_shape[2],
...
std::string convertInstructionToString(ChangeInstruction instruction){
    std::string result = "layer_idx: "+std::to_string( instruction.layer_idx)+" \t type: "+std::to_string( (int) instruction.type)+" \t source_rank: "+std::to_string( instruction.source_rank)+" \t target_rank: "+std::to_string( instruction.target_rank)+" \t source_global_position: "+std::to_string( instruction.source_global_position)+" \t target_global_position: "+std::to_string( instruction.target_global_position)+ " \t source_expert_id: "+std::to_string( instruction.source_expert_id)+" \t target_expert_id: "+std::to_string( instruction.target_expert_id) + "\n";
    return result;
}
...
void Placement::placement_manager(aclrtContext currentContext) {
    ...
    while(!moe_weight_->isHbmInitialized()){
                std::this_thread::sleep_for(std::chrono::seconds(1)); // Run every 1 mins
    }
        size_t expert_size = moe_weight_->get_expert_size(); // 根据QueueSize 预分配 接受buffs
    ...
    size_t collect_times = 60; // 6 mins  to collect information
     ...
     activations_->collect(dist_ptr, stream); // update the last & delta activation
    ...
    while(!should_stop_) {
        ...
        std::string log_info = "";
        // 构建下发交换队列
        std::vector<ChangeInstruction> changeInstructions;  // all rank total instructions
        ...
        for (auto& instruction : changeInstructions)
        {
            ...
            if (instruction.source_rank !=rank_ && instruction.target_rank!=rank_) continue;
            if (instruction.source_rank ==rank_ && instruction.type == OperationType::REMOVE) continue;
            changeInstructions_this_rank.push_back(instruction);

            // Log Infomation
            if (instruction.source_rank==rank_){
                log_info += " S-L["+std::to_string(instruction.layer_idx)+"]-T["+std::to_string((int)instruction.type)+"]-TR[" + std::to_string(instruction.target_rank)+"]";
            }
            else if (instruction.target_rank==rank_){
                log_info += " T-L["+std::to_string(instruction.layer_idx)+"]-T["+std::to_string((int)instruction.type)+"]-TR[" + std::to_string(instruction.target_rank)+"]";
            }
            
        }


        std::string log_tile = "Rank: "+ std::to_string(rank_) + " , Exchanged Info: with nums["+std::to_string(changeInstructions_this_rank.size()) + "] \n";
        ...
        while(idx<changeInstructions_this_rank.size()) {
            ...
            int t_rank = (instruction.source_rank == rank_) ? instruction.target_rank : instruction.source_rank;
            int global_position_id_this_layer = (instruction.source_rank == rank_) ? instruction.source_global_position : instruction.target_global_position;
            int expert_id = (instruction.source_rank == rank_) ? instruction.target_expert_id : instruction.source_expert_id;

            TRACK_POINT("tag1, Rank: " + rank_str);
        
            
            // int expert_id = (instruction.rank_a == rank_) ? instruction.expert_idx_a : instruction.expert_idx_b; // UnitTest: 不要交换收益

            int position_offset = mapping->getGlobalPositionOffset(layer,global_position_id_this_layer);
            TRACK_POINT("getGlobalPositionOffset tag2, Rank: " + rank_str);

            if (type==OperationType::ADD && instruction.source_rank == rank_){
                need_enqueue_recv_buff=false;
                position_offset = -1; // add的source端不更新
            }

            if (type==OperationType::REMOVE && instruction.target_rank == rank_){
                t_rank = rank_; // 自己跟自己握手
                expert_id = -1; // 告诉该位置专家id修改为-1
            }

            
            bool flag = dist_ptr->performGlobalHandshake(t_rank,position_offset,expert_id); // 对端握手成功， 准备下发
            // mapping 有一把琐 要加在 deployed_mapping更新， 且保证权重入队完成后才解锁
            TRACK_POINT("performGlobalHandshake, Rank: " + rank_str);
            

            if (flag){
                // std::cout<<getTimestamp()<< "Do Rank: "<<rank_<<" idx["<<idx<<"/"<<changeInstructions_this_rank.size()<<"] fail_handshake_count:"<< fail_handshake_count <<std::endl;
                fail_handshake_count = 0;
                bool positionIsConsistency;
                if (type==OperationType::SWAP){
                    positionIsConsistency = mapping->checkPositionIsConsistency(layer,instruction.source_global_position,instruction.source_expert_id);
                    if(!positionIsConsistency){
                        throw std::runtime_error("[Error]-rank["+std::to_string(rank_)+"]  positionIsConsistency");
                        // return;
                    }
                    positionIsConsistency = mapping->checkPositionIsConsistency(layer,instruction.target_global_position,instruction.target_expert_id);
                    if(!positionIsConsistency){
                        throw std::runtime_error("[Error]-rank["+std::to_string(rank_)+"]  positionIsConsistency");
                        // break;
                    }
            ...
            bool isUpdateValied = mapping->update_globalDeployedPositionToLogisticsIdMapping(dist_ptr->getHandshakeStatus(),dist_ptr->get_info_length(),is_layer_update);
            if(! isUpdateValied){
                std::cout<<"[Error]-isUpdateValied \t layer_idx: "<< instruction.layer_idx<<" \t type: "<< (int) instruction.type<<" \t source_rank: "<< instruction.source_rank<<" \t target_rank: "<< instruction.target_rank<<" \t source_global_position: "<< instruction.source_global_position<<" \t target_global_position: "<< instruction.target_global_position<<" \t source_expert_id: "<< instruction.source_expert_id<<" \t target_expert_id: "<< instruction.target_expert_id<<std::endl;
                throw std::runtime_error("[Error]-rank["+std::to_string(rank_)+"] \t isUpdateValied: false");
            } 
            ...
            TRACK_POINT("update_globalDeployedPositionToLogisticsIdMapping, Rank: " + rank_str);

            if (flag){
                void* recv_buff_address = dist_ptr->get_recv_buff_address();
                moe_weights->replacement(dist_ptr, layer, instruction.source_rank,instruction.source_global_position,instruction.target_rank,instruction.target_global_position,recv_buff_address,need_enqueue_recv_buff);
                // lock.unlock();
                sub_thread_is_changing_=false;
                idx++;
                ...
                // std::cout<<"\t rank["<<rank_<<"] handshake failed with t_rank["<<t_rank<<"]"<<std::endl;
                // lock.unlock();
                sub_thread_is_changing_=false;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
            }
            // std::cout<<"rank["<<rank_<<"], queue_size["<<dist_ptr->getCompletedQueueSize()<<"]"<<std::endl;
        ...
        while(true){
            if (should_stop_) break;
            // std::cout<<"rank["<<rank_<<"] idx["<<idx<<"]-total["<<changeInstructions_this_rank.size()<<"]"<<std::endl;
            bool flag = dist_ptr->performGlobalHandshake(-1,-1,-1); //所有rank 都是-1， return true
            std::unique_lock<std::mutex> lock = acquire_lock();
            sub_thread_is_changing_=true;
            lock.unlock();
            bool isUpdateValied = mapping->update_globalDeployedPositionToLogisticsIdMapping(dist_ptr->getHandshakeStatus(),dist_ptr->get_info_length(),is_layer_update);
            sub_thread_is_changing_=false;
            if (!isUpdateValied) throw std::runtime_error("[Error]-rank["+std::to_string(rank_)+"] \t isUpdateValied: false");
            if (flag) break;
            std::this_thread::sleep_for(std::chrono::milliseconds(10));  // wait other ranks
        }
        TRACK_POINT("wait all rank finish Optimize, Rank: " + rank_str);
        std::cout<< getTimestamp() << "rank["<<rank_<<"] finished"<<std::endl;

        activations_->collect(dist_ptr, stream); // Clear the old placement activations
        std::this_thread::sleep_for(std::chrono::seconds(collect_times));  // wait other ranks
    }
...
void Placement::start_thread() {
    if (!worker_thread_.joinable()) {
        ...
        worker_thread_ = std::thread(&Placement::placement_manager, this,currentContext);
    }
}
...
void do_placement_optimizer(Placement& placement) {
    ...
    if (!placement.get_subthread_is_changing()){
        // TIME_IT_LABEL("-------------------copyAllFromCompletedQueueToHBM--rank_"+std::to_string(placement.get_rank()),{
        dist_ptr->copyAllFromCompletedQueueToHBM();
        // });
        // TIME_IT_LABEL("update_globalLogisticsIdToDeployedPositionMapping--rank_"+std::to_string(placement.get_rank()),{
...
PYBIND11_MODULE(omni_placement, m) {
    ...
    m.def("do_placement_optimizer", &do_placement_optimizer, py::arg("placement"));
    ...
    py::class_<PlacementMapping>(m, "PlacementMapping")
        .def(py::init<const std::string& , int , int , int , int ,
                        ...
                        size_t>(),
        py::arg("filename"), py::arg("rank"), py::arg("num_devices"), py::arg("max_deployed_num"), py::arg("max_deployed_num"),
    ...
    py::class_<ClusterActivation>(m, "ClusterActivation")
        .def(py::init<Tensor,int64_t,size_t, size_t, int, size_t, size_t>(), // 按实际构造函数参数补全
        py::arg("npu_count"),
        py::arg("max_activation_count"),
        py::arg("layer"),
        py::arg("num_expert"),
        py::arg("window_size"),
        py::arg("world_size"),
