...
do { \
    ...
    std::cout << "Time taken by [" << label << "]: " << duration << " milliseconds" << std::endl; \
} while (0)
    ...
    public:
        ...
        void Enqueue(TransDesc* desc){
            ...
            if (desc==nullptr){
                ...
                transDesc->localExpertPositionOfsset = desc->localExpertPositionOfsset;
            }
            currentSize++;
            lock.unlock();
            mCond.notify_one();
        ...
        {
            if (IsEmpty()) {
                std::cout<<"The Swap Queue is Empty, Ignore the Dequeue Operation!";
                return;
            }
            std::unique_lock<std::mutex> lock(mMutex);
        ...
        TransDesc * GetRear(size_t position_offset){
            ...
            for (int i = 0; i < currentSize; ++i) {
                ...
                if (mTransDescQueue[index]->localExpertPositionOfsset == position_offset) {
                    result = mTransDescQueue[index];
                }
            }
            return result; // 如果没有找到，返回null
        }
        size_t getEnqueueIdx(){
            return (mCurDescRear + 1) % QUEUE_SIZE;
        }
...
class Distribution{
    private:
        ...
        size_t info_length_=3; // 握手信息的信息量 <t_rank, global_position_id, expert_id>;
    ...
    public:
        ...
        ~Distribution();
        void enqueue(TransDesc* desc,size_t t_rank, bool need_enqueue_recv_buff); // 选择放到哪个队列当中
        void swap(void * src_addr, void * recv_addr,size_t length, std::string dtype, uint32_t t_rank, bool send_first, aclrtStream stream);
        void allgather(void * src_addr, void * recv_addr,size_t length, std::string dtype);
        void printCommInfo();
        void warmup();
        ...
        size_t getCompletedQueueEnqueuePosition()  { return completedSynchronizeQueue_.getEnqueueIdx();}
        ...
        int getCompletedQueueSize() const {return completedSynchronizeQueue_.getCurrentSize();}
