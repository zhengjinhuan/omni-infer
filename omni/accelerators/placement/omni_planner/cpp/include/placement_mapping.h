...
protected:
    ...
    int32_t max_redundant_per_rank_ ; // The number of redundant expert of each Rank
    ...
    std::vector<int32_t> globalDeployedPositionToLogisticsIdMappingHost_; // DRAM-全局部署专家对应逻辑专家映射 shape: [num_layers, world_size_,  num_deploy_experts_per_device_]
...
private:
    ...
    void init_placement_pattern(const std::string& placement_pattern_filename, size_t placement_pattern_ptr, std::vector<int64_t> placement_shape);


public:
    /**
     * @brief Constructor for PlacementMapping
     * @param placement_pattern_filename Path to binary file containing placement pattern (used if placement_pattern_ptr is empty)
    ...
    * @param dist_ptr 通信工具
    */
    bool update_globalDeployedPositionToLogisticsIdMapping(std::vector<int> global_synchronize_mapping_info, size_t num_info, std::vector<bool>& is_layer_update); // 子线程调用
    bool checkUpdateIsValied(size_t layer_id, int expert_id, int ops);

    int getGlobalPositionOffset(int layer_id, int global_position_id_this_layer );
    int get_num_redundant_per_rank() {return  max_redundant_per_rank_;}
    void printDeployedPositionToLogisticsIdMapping();
    bool checkPositionIsConsistency(size_t layer_id, size_t global_position, int expert_id);

    // 新增 getter 方法
    std::vector<int32_t> get_global_deployed_position_to_logistics_id_mapping() const {
        return globalDeployedPositionToLogisticsIdMappingHost_;
    }

    void update_globalDeployedPositionToLogisticsIdMapping(int layer_id, size_t offset, int expert_id);

    int32_t get_expert_id(int layer_id, int global_position_id_this_layer) {
        size_t global_offset = getGlobalPositionOffset(layer_id, global_position_id_this_layer);
        return globalDeployedPositionToLogisticsIdMappingHost_[global_offset];
    }

    int get_max_redundant_per_expert() const { return max_redundant_per_expert_; }
    void update_selector(std::vector<bool> is_layer_update);
    void update_selector_layer(int layer_id,std::vector<int> finish_table,std::vector<std::vector<int32_t>> same_host_candidates,std::vector<std::vector<int32_t>> distant_candidates);
