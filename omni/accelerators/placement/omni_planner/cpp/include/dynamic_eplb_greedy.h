    ...
    public:
        ...
        void update(int global_position_id, int value,float keep_ratio){
            ...
            if (idx==-1){
                ...
                activated_values[idx] = (1-keep_ratio)*value + activated_values[idx]*keep_ratio;
            }
        }
        ...
        void update(){
            ...
            if (activated_values.size()<=1) throw std::runtime_error("Invalid activated_values.size() is less than 2 , could split");
        ...
        bool is_redundant(){
            return global_position_ids.size()>1;
        }

        int find_position(int global_position_id){
            for(size_t idx=0;idx<global_position_ids.size();++idx){
                if (global_position_ids[idx]==global_position_id) return idx;
            }
            return -1;
        }

        int getNumRedundants() const{
            return  global_position_ids.size();
        }
        ...
        std::vector<int> getLocalPositionIdx(int num_deployed_expert_per_rank){
            std::vector<int> results;
            for(size_t idx=0;idx<global_position_ids.size();++idx){
                int localpositionidx = global_position_ids[idx] % num_deployed_expert_per_rank;
                results.emplace_back(localpositionidx);
            }
            return results;
        }

        std::vector<int> getGlobalPositionIdx(){
            return global_position_ids;
        }

        int getActivateValue(int global_position_id){
            int idx = find_position(global_position_id);
            if (idx == -1) {
                throw std::runtime_error("this logit expert is not deployed on global_position_per_layer[" + std::to_string(global_position_id) + "]");
            }
...
class RankActivateInformation{
    ...
    public:
        ...
        int getNumPosition() const{
            return deployed_to_logits_mapping_.size();
        }
        void update(int position_id, int expert_id, int value){
            ...
            {
                throw std::runtime_error("expert_id is -1 while value["+std::to_string(value)+"] is not equal to 0");
            }
...
class GreedyExpertLoadBalancer {
    private:
        ...
        int rank_;
        std::vector<LogitExpertInformation*> logit_expert_info_ptrs_; // 所有layer的逻辑专家部署以及激活信息
        std::vector<RankActivateInformation*> rankinfo_ptrs_; // 所有layer的各个Rank的激活信息
        float alpha_=0.6; //0.6; // activation 保留速率
        void init_infomation(int layer_id,std::vector<int> placement, std::vector<int64_t> activations );
        ...
        int getTheHighestOffload(std::vector<int> subset_ranks_values, int bestEPValue, std::vector<int> include_ranks);
        std::vector<ChangeInstruction> generate_remove_instructions_per_layer(int layer_id, int bestEP_value);
        RankActivateInformation* getTheHighestOffloadRank(int layer_id, std::vector<int> exclude_ranks);
        RankActivateInformation* getTheLowestOffloadRank(int layer_id, std::vector<int> exclude_ranks);
        ...
        std::vector<ChangeInstruction> generate_swap_add_instructions_per_layer(int layer_id);
        bool isInvalidOffloadRatio(float ratio) const { return ratio >300;}


    public:
        GreedyExpertLoadBalancer(int num_layers, int world_size, int num_experts,int num_deployed_experts, int expert_redundant_limit, int rank);
        ~GreedyExpertLoadBalancer();
        std::vector<ChangeInstruction> optimize_and_generate_instructions(std::vector<int> placement,std::vector<int64_t> activations);
};
