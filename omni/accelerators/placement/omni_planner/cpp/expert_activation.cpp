...
ClusterActivation::ClusterActivation(Tensor npu_count, int64_t max_activation_count, size_t num_layers, size_t num_deploy_experts_per_rank, int activation_window_size,size_t world_size, size_t rank)
    : npu_count_(npu_count),max_activation_count_(max_activation_count),num_layers_(num_layers), num_deploy_experts_per_rank_(num_deploy_experts_per_rank), activation_window_size_(activation_window_size), world_size_(world_size),rank_(rank)
{
    ...
    // Since local tokens global experts -> glocal tokens local experts, npu_coun_.get_length() must be less than get_num_layers()*(size_t)get_num_deploy_experts_per_rank()
    if (npu_count_.get_length() > get_num_layers() * (size_t)get_num_deploy_experts_per_rank()) {
        throw std::runtime_error("npu_count's length is: "+std::to_string(npu_count_.get_length())+" , which is larger than Current total layer and experts_per_layer is :"+ std::to_string(get_num_layers()) +"*"+std::to_string(get_num_deploy_experts_per_rank()));
    }
...
void ClusterActivation::init_activation_hbm() {
    ...
    expert_activation_counts_ = new Tensor(static_cast<uint64_t*>(data_ptr), length, sizeof(int64_t), "expert_activation_counts_");
...
void ClusterActivation::dumpActivationCountsPerRank(size_t dump_count, int64_t* total_count_ptr){
    std::cout<<"EXPERT_COUNTS For RecordStep:"<<std::to_string(dump_count)<<std::endl;
    ...
    for (size_t layer_id = 0; layer_id < num_layers_; ++layer_id) {
        for (size_t j = 0; j < num_deploy_experts_per_rank_; ++j) {
            ...
            int64_t countDiff = total_count_ptr[offset]; // 旧接口， 依旧保留使用，无滑动窗口
            oss << countDiff << "\t";
        }
        oss << std::endl;
    }
    ...
    if (!dump_dir_.empty()) {
        std::string filename = dump_dir_ + "_perRank/activation_counts_recordstep_" + std::to_string(dump_count) + "_rank_"+std::to_string(get_rank())+".txt";
    ...
    for (size_t layer_id = 0; layer_id < num_layers_; ++layer_id) {
        for (size_t rank_id=0; rank_id<world_size_;++rank_id) {
            for (size_t j = 0; j < num_deploy_experts_per_rank_; ++j) {
                ...
                int64_t countDiff = getExpertActivationCount(layer_id, physical_pos, delta_experts_counts_); // 旧接口， 依旧保留使用，无滑动窗口
                oss << countDiff << "\t";
            }
        }
        oss << std::endl;
...
void ClusterActivation::collect(Distribution* dist_ptr, aclrtStream stream){
    ...
    // Hccl AllGather
    // std::cout<<"rank_["<<rank_<<"] Start!!!!! activation info allgather Ops!"<<std::endl;
    dist_ptr->allgather(npu_count_.get_data_ptr(),expert_activation_counts_->get_data_ptr(),npu_count_.get_length(),npu_count_.get_dtype());
...
void ClusterActivation::dump_and_collect(Distribution* dist_ptr, aclrtStream stream, size_t dump_count) {
...
void ClusterActivation::updateDeltaActivationCount(){
    size_t total_size = world_size_ * num_layers_ * num_deploy_experts_per_rank_;
    int64_t* counts_host = static_cast<int64_t*>(deployed_experts_counts_host_);
    ...
    for (size_t offset = 0; offset < total_size; ++offset) {
        ...
        if (current < last) current += max_activation_count_;
        delta_counts[offset] = current - last;
    }
    // Copy deployed_experts_counts_host_ to last_count_ptr_ using memcpy
    std::memcpy(last_count_ptr_, deployed_experts_counts_host_, total_size * sizeof(int64_t));
}
void ClusterActivation::updateShiftWindows(PlacementMapping* placement_mapping){
    for (int layer_id = 0; layer_id < placement_mapping->get_num_layers(); ++layer_id) {
        std::vector<int64_t> logit_expert_activations(num_experts_,0);
        int64_t total_value = 0;
        for (int global_position_id = 0; global_position_id<placement_mapping->get_num_deploy_experts();++global_position_id){
            int32_t expert_id = placement_mapping->get_expert_id(layer_id, global_position_id);
            int64_t value = getExpertActivationCount(layer_id, global_position_id, delta_experts_counts_);
            total_value = total_value + value;
            if(expert_id==-1) continue;
            logit_expert_activations[expert_id] = logit_expert_activations[expert_id] + value;
        }

        if (total_value != 0){
            for (size_t idx=0; idx< num_experts_;++idx){
                size_t offset = layer_id*num_experts_+idx;
                all_logit_experts_activation_[offset].update(logit_expert_activations[idx]);
                // if(rank_==0) std::cout<<"offset["<<offset<<"] expert_id["<<idx<<"] value["<<all_logit_experts_activation_[offset].get_last_value()<<"] ";
            }
        }
    }
}
...
int64_t ClusterActivation::getLayerActivationCount(size_t layer_idx) {
    if (layer_idx >= num_layers_ ) {
        throw std::runtime_error("Invalid layer or expert, which is: layer: "+std::to_string(layer_idx)+" : "+ std::to_string( num_layers_));
    }
    
    int64_t layer_counts = 0;
    int64_t* deployed_experts_counts_host = static_cast<int64_t*>(deployed_experts_counts_host_);
    for (size_t rank = 0; rank < world_size_; rank++) {
        for (size_t pos = 0; pos < num_deploy_experts_per_rank_; pos++) {
            size_t idx = rank * num_layers_ * num_deploy_experts_per_rank_ +  layer_idx * num_deploy_experts_per_rank_ + pos;
...
int64_t ClusterActivation::getClusterTotalActivationCount(size_t layer_idx, size_t deploy_expert_idx) {
    ...
    if (layer_idx >= num_layers_ || deploy_expert_idx >= num_deploy_experts_) {
        throw std::runtime_error("Invalid layer or expert, which is: layer: "+std::to_string(layer_idx)+" : "+ std::to_string( num_layers_)+" expert_idx: "+std::to_string(deploy_expert_idx)+" : "+std::to_string( num_deploy_experts_));
    }
    return getExpertActivationCount(layer_idx, deploy_expert_idx, delta_experts_counts_);
}

// 读取 expert_activation_counts_[layer][expert] 的值
int64_t ClusterActivation::getExpertActivationCount(int32_t layer_id, int32_t expert_id){
    return getExpertActivationCount(layer_id,expert_id,delta_experts_counts_);
}
int64_t ClusterActivation::getExpertActivationCount(int32_t layer_id, int32_t expert_id, void* activation_ptr) {
    ...
    size_t total_offset = t_rank*num_layers_*num_deploy_experts_per_rank_+layer_id*num_deploy_experts_per_rank_+local_expert_idx;
...
int64_t ClusterActivation::getLogitExpertShiftActivateion(int32_t layer_id, int32_t expert_id, int this_expert_deployed_num) {
...
void ClusterActivation::collect_from_txt(const std::string& txt_path){
    ...
    while (std::getline(file, line) && layer_id < 58) {
        ...
        while (std::getline(iss, token, '\t')) {
            ...
            size_t offset = rank * num_layers_* num_deploy_experts_per_rank_ + layer_id * num_deploy_experts_per_rank_ + pos_id%num_deploy_experts_per_rank_;
...
void ClusterActivation::print_activations() {
    ...
    // for (size_t i = 0; i < num_layers_ * num_deploy_experts_per_rank_; ++i) {
