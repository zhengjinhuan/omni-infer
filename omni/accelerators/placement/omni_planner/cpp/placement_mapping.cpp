...
PlacementMapping::PlacementMapping(const std::string& placement_pattern_filename, int rank,
    int num_devices_per_host, int max_redundant_per_expert, int max_num_deployed_expert,
    ...
    max_redundant_per_expert_(max_redundant_per_expert)
{
    ...
    init_placement_pattern(placement_pattern_filename, placement_pattern_ptr, placement_shape);
    ...
    max_redundant_per_rank_ = num_deploy_experts_per_device_-num_experts_per_device_;
...
void PlacementMapping::init_placement_pattern(const std::string& placement_pattern_filename, size_t placement_pattern_ptr, std::vector<int64_t> placement_shape){
    // 检查是否提供了文件名且指针为空
    bool use_file = (!placement_pattern_filename.empty() && placement_pattern_filename[0] != '\0');
    if (use_file) {
...
std::vector<std::vector<std::vector<int>>> PlacementMapping::load_placement_pattern_from_file(const char* filename) {
    ...
    if (!file.is_open()) {
        throw std::runtime_error("Failed to open placement pattern file: " + std::string(filename));
    }
    ...
    long long expected_count = static_cast<long long>(num_devices) * num_layers * num_experts;
    ...
    for (int i = 0; i < num_devices; ++i) {
        for (int j = 0; j < num_layers; ++j) {
            for (int k = 0; k < num_experts; ++k) {
                if (!(file >> value)) {
                    file.close();
                    throw std::runtime_error("Failed to read sufficient placement pattern data; file may be incomplete");
                }
                if (value != 0 && value != 1) {
                    file.close();
                    throw std::runtime_error("Placement pattern contains invalid value (not 0 or 1)");
                }
                pattern[i][j][k] = value;
                ++read_count;
            }
    ...
    if (file >> value) {
        std::cerr << "Warning: File contains excess data, not read" << std::endl;
    }

    // Verify read count
    if (read_count != expected_count) {
        file.close();
        throw std::runtime_error("Number of read values does not match expected shape");
    }
...
int32_t PlacementMapping::get_position_expert_id(int32_t layer_id, int32_t position_id){
    // 边界检查
    ...
    if (position_id < 0 || position_id >= get_num_deploy_experts()) {
        throw std::out_of_range("position out of range: " + std::to_string(get_num_deploy_experts()));
    }
        ...
        for (int32_t rank = 0; rank < world_size_; ++rank) {
            ...
            for (int32_t expert = 0; expert < num_experts_; ++expert) {
                if (placement_pattern_vector_[rank][layer][expert] == 1) {
                    ...
                    // 确保冗余计数不超过最大允许值
                    assert(expert_id_deployed_nums_[layer*num_experts_+expert]  < max_redundant_per_expert_);

                    // 计算全局位置ID：使用设备编号乘以每设备最大专家数作为基础偏移，再加上当前位置
                    int32_t global_position_id_per_layer = num_deploy_experts_per_device_ * rank + local_position_id;   // offset + cumsum;
                    size_t offset = layer*num_deploy_experts_+global_position_id_per_layer;

                    // Set the expert_id on offset Position
                    globalDeployedPositionToLogisticsIdMappingHost_[offset] = expert;
...
void PlacementMapping::printDeployedPositionToLogisticsIdMapping() {
    // 打印 vector 的大小
    std::cout << "globalDeployedPositionToLogisticsIdMappingHost_ size: " << globalDeployedPositionToLogisticsIdMappingHost_.size() << std::endl;
    ...
    for (int32_t layer_id = 0; layer_id < num_layers_; ++layer_id) {
        for (int32_t position_id_this_layer = 0; position_id_this_layer < num_deploy_experts_; ++position_id_this_layer) {
            int offset = getGlobalPositionOffset(layer_id,position_id_this_layer);
            log_info += std::to_string(position_id_this_layer)+":"+ std::to_string(globalDeployedPositionToLogisticsIdMappingHost_[offset])+", ";
        }
        log_info += "\n";
        break;
    }
...
void PlacementMapping::update_globalDeployedPositionToLogisticsIdMapping(int layer_id, size_t offset, int expert_id) {
    if (globalDeployedPositionToLogisticsIdMappingHost_[offset]!=-1){
        checkUpdateIsValied(layer_id, globalDeployedPositionToLogisticsIdMappingHost_[offset], -1);
    }
    if (expert_id!=-1){
...
bool PlacementMapping::checkPositionIsConsistency(size_t layer_id, size_t global_position, int expert_id){
    size_t position_offset = layer_id * get_num_deploy_experts()+global_position;
    if(globalDeployedPositionToLogisticsIdMappingHost_[position_offset]== expert_id){
        return true;
    }
    else{
        std::cout<<"[Error]-layer["<<layer_id<<"]-global_position["<<global_position<<"]-expert_id["<<globalDeployedPositionToLogisticsIdMappingHost_[position_offset]<<"]-InstructionExpertId["<<expert_id<<"]\n";
        return false;
    }
}

bool PlacementMapping::checkUpdateIsValied(size_t layer_id, int expert_id, int ops){
    // 判断更新是否符合
    if (expert_id==-1) throw std::runtime_error("[Error]-rank["+std::to_string(rank_)+"]  checkUpdateIsValied");
    size_t offset = get_num_experts() * layer_id +expert_id;
    expert_id_deployed_nums_[offset] += ops;
    if (expert_id_deployed_nums_[offset]<0) {
        std::cout<<"[Error] rank["<<rank_<<"] on Deployed Mapping Update !!!!!!!!!! with\t layer_id["<<layer_id<<"] expert_id["<< expert_id<<"] nums["<<expert_id_deployed_nums_[offset]<<"]"<<std::endl;;
        return false;
    }
    return true;
}

bool PlacementMapping::update_globalDeployedPositionToLogisticsIdMapping(std::vector<int> global_synchronize_mapping_info, size_t num_info, std::vector<bool>& is_layer_update){
    bool isUpdateValied;
    for (int32_t rank = 0; rank < world_size_; ++rank) {
        ...
        if (t_rank==-1 || t_rank==world_size_) continue;
        // 对端与当前Rank握手，才发生更新
        if(position_offset!=-1 && global_synchronize_mapping_info[t_rank*num_info] == rank){
            size_t layer_id = position_offset / get_num_deploy_experts();
            is_layer_update[layer_id] = true;
            update_globalDeployedPositionToLogisticsIdMapping(layer_id, position_offset,expert_id);
        }
    }
    return true;
    // if(rank_==0||rank_==16) printDeployedPositionToLogisticsIdMapping();
}

int PlacementMapping::getGlobalPositionOffset(int layer_id, int global_position_id_this_layer ){
...
void PlacementMapping::init_selector(size_t selector_ptr){
    ...
    selector_ = Tensor((uint64_t) selector_ptr, num_layers_*num_experts_, sizeof(int32_t), "int32_t","selector");
    std::vector<bool> tmp(num_layers_,true);
    update_selector(tmp);
}

void PlacementMapping::update_selector_layer(int layer_id, std::vector<int> finish_table, std::vector<std::vector<int32_t>> same_host_candidates,std::vector<std::vector<int32_t>> distant_candidates)
{
    ...
    for (int pos_id = 0; pos_id < num_deploy_experts_; ++pos_id) {
        ...
        int  logit_expert_id = globalDeployedPositionToLogisticsIdMappingHost_[position_offset+pos_id];

        if (logit_expert_id==-1) continue;
        ...
        if (finish_table[logit_expert_id]==2) continue;
        ...
        if (phy_device == rank_){
            ...
            selector_host_[logit_expert_id+logits_offset] = same_host_candidates[logit_expert_id][rank_%same_host_candidates[logit_expert_id].size()];
            finish_table[logit_expert_id] = 1; 
        }
        else if (finish_table[logit_expert_id] < 1){
            distant_candidates[logit_expert_id].push_back(pos_id);
            selector_host_[logit_expert_id+logits_offset] = distant_candidates[logit_expert_id][rank_%distant_candidates[logit_expert_id].size()];
        }
    }
}
...
void PlacementMapping::update_selector(std::vector<bool> is_layer_update){
    ...
    std::vector<std::vector<int32_t>> same_host_candidates(num_experts_, std::vector<int32_t>());
    std::vector<std::vector<int32_t>> distant_candidates(num_experts_, std::vector<int32_t>());
    ...
    // TIME_IT_LABEL("-------------update_selector_layer--rank_"+std::to_string(rank_),{
    for(int layer_id=0; layer_id<num_layers_; ++layer_id){
        if (is_layer_update[layer_id]) update_selector_layer(layer_id,finish_table,same_host_candidates,distant_candidates);
    }
    // });

    // TIME_IT_LABEL("-------------selector_host_to_device--rank_"+std::to_string(rank_),{
