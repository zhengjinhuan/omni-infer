...
class OmniPlanner(metaclass=OmniPlannerMeta):
    ...
    def __init__(self, config_file: str = "/etc/omni/config.yaml", device: str = "npu",
        ...
        self.max_num_deployed_expert_per_rank = max(self.get_deployed_experts_per_layer())//self.world_size
        ...
        self.num_logits_expert_per_rank = self.expert_mapping.get_total_num_expert()//self.world_size
    ...
    def place_experts(self, layer_idx_moe: Optional[int] = None) :
        ...
        self._step += 1
        if self._step % 100 == 0:
            do_placement_optimizer(self.placement_manager, layer_idx_moe)
    ...
    def plan(
        ...
        #TODO:triger by vllm forward
        # self.place_experts(layer_idx_moe)
        return tokens, token_expert_ids, token_expert_scores
    ...
    def dump(self,step):
        ...
        if not enable_dump:
            ...
            return
        if dump_dir is None:
            raise ValueError("dump_dir must not be None, Pls Set dump_dir")
        if step==0:
