...
def deepseek_get_layer_idx_func(key,first_k_dense_replace=3):
    ...
    match = re.match(pattern, key)
    if not match:
        raise ValueError(f"current key is {key}, layer.layer_idx")
    layer = int(match.group(1))  # 提取layer数字
    return layer - first_k_dense_replace

@calculate_time
def init_dram_weights(moe_weights,param_dict,local_rank_pattern,first_k_dense_replace,init_shm):
    """
    Args:
    ...
    moe_weights.init_weights(ctype_param_list,experts_id_list,init_shm)
...
def create_cluster_activation(rank, world_size, num_layers,num_deploy_experts_per_rank, count_activation):
    """
...
def get_hccl_root_info(rank) :
    ...
    print(f"Rank {rank}: Broadcasted rootinfo = {root_info[:16]}... (length={len(root_info)})")
    return root_info