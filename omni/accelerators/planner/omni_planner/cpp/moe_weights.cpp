...
void ExpertWeights::enqueueSwapInformation(Distribution* dist_ptr, size_t layer_idx, SwapInstruction instruction){
    ...
    expert_trans_desc.layer_idx = layer_idx;
    expert_trans_desc.instruction = instruction;
    dist_ptr->enqueue(&expert_trans_desc);
}
...
MoEWeights::MoEWeights(size_t num_experts, size_t rank, size_t world_size,const char* rankTableFile):rank_(rank),world_size_(world_size),num_experts_(num_experts){
    ...
    // TODO: 判断传入的rank 与 worldsize 与 distribution通信域的是否一致
    shm_ptr_ = nullptr;
    count_ptr_ = nullptr;
    num_deploy_experts_per_rank_ = num_experts_/world_size_;
    shm_unlink(shm_name_.c_str());
...
void MoEWeights::init_weights(const std::vector<std::vector<std::vector<Tensor>>>& npu_weights, const std::vector<std::vector<int>>& expert_ids, bool init_shm) {
...
void MoEWeights::replacement(Distribution* dist_ptr, size_t layer_idx, SwapInstruction instruction){
    ExpertWeights expert = getExpert(layer_idx, instruction.expert_position_a%getNumDeployExpertsPerRank());
    expert.enqueueSwapInformation(dist_ptr, layer_idx, instruction); // 往队里传入专家替换信息， 异步线程处理,不进行同步等待, TODO 
}
