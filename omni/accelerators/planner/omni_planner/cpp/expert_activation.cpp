...
ClusterActivation::ClusterActivation(Tensor npu_count,size_t num_layers, size_t num_deploy_experts_per_rank, int activation_window_size,size_t world_size, size_t rank)
    : npu_count_(npu_count),num_layers_(num_layers), num_deploy_experts_per_rank_(num_deploy_experts_per_rank), activation_window_size_(activation_window_size), world_size_(world_size),rank_(rank)
{
...
void ClusterActivation::init_activation_hbm() {
    ...
    size_t act_hbm_size = num_layers_ * num_deploy_experts_ * sizeof(int64_t);
    auto result = aclrtMalloc((void**)&expert_activation_counts_, act_hbm_size, ACL_MEM_MALLOC_HUGE_FIRST);
    if (result != ACL_ERROR_NONE) {
        std::cout << "alloc expert activation counts memory failed, size " << act_hbm_size << " error code " << result << std::endl;
        throw std::runtime_error("alloc expert activation counts memory failed.");
    }
    aclrtMemset((void*)expert_activation_counts_, act_hbm_size, 0, act_hbm_size);
}

void ClusterActivation::free_activation_hbm() {
    aclrtFree(expert_activation_counts_);
}

// 初始化激活共享内存
void ClusterActivation::init_activation_shmem() {
    act_shm_size_ = world_size_*num_layers_ * num_deploy_experts_per_rank_ * sizeof(ExpertActivation);
    ...
    // 改为只初始当前rank这块，
    for (size_t i =rank_*num_layers_ * num_deploy_experts_per_rank_; i < (rank_+1)*num_layers_ * num_deploy_experts_per_rank_; i++)
    {
...
void ClusterActivation::dumpActivationCounts(size_t dump_count, int64_t* total_count_ptr, int64_t* last_count_ptr) {
    ...
    for (size_t i = 0; i < num_layers_; ++i) {
        for (size_t j = 0; j < num_deploy_experts_per_rank_; ++j) {
            size_t index = i * num_deploy_experts_per_rank_ + j;
...
void ClusterActivation::collect_wrapper(HcclComm comm, aclrtStream stream) {
    size_t dump_count = 0;
    //while(thread_state_ == ThreadState::RUNNING) {
        ...
        collect_activation(comm, stream);
        /*
...
void ClusterActivation::collect_activation(size_t layer_idx, size_t deploy_expert_idx, int64_t count) {
    ...
    setExpertActivationCount(layer_idx, deploy_expert_idx, count);
}

void ClusterActivation::collect_activation(HcclComm comm, aclrtStream stream) {
    allGatherActivation(comm, stream);
}

// Get activations
int64_t ClusterActivation::getClusterTotalActivationCount(size_t layer_idx, size_t deploy_expert_idx) {
    if (layer_idx >= num_layers_ || deploy_expert_idx >= num_deploy_experts_per_rank_) {
        throw std::runtime_error("Invalid layer or expert");
    }
    /*
    size_t idx = 0;
    ...
    {
        idx = i* num_layers_ * num_deploy_experts_per_rank_ +  layer_idx * num_deploy_experts_per_rank_ + deploy_expert_idx;
...
int64_t ClusterActivation::getExpertActivationCount(int32_t layer_id, int32_t expert_id) {
    ...
    int32_t offset = layer_id * num_deploy_experts_ + expert_id;
    int64_t value;
    aclError ret = aclrtMemcpy(
        &value,                                  // Host destination
        sizeof(int64_t),
        expert_activation_counts_ + offset,    // NPU source
        sizeof(int64_t),
        ACL_MEMCPY_DEVICE_TO_HOST
    );
    if (ret != ACL_ERROR_NONE) {
        throw std::runtime_error("aclrtMemcpy failed in get_expert_activation_count at layer " +
...
void ClusterActivation::setExpertActivationCount(int32_t layer_id, int32_t expert_id, int64_t value) {
    if (layer_id < 0 || layer_id >= (int32_t)num_layers_) {
        throw std::out_of_range("layer_id out of range: " + std::to_string(layer_id));
    }
    if (expert_id < 0 || expert_id >= (int32_t)num_deploy_experts_) {
    ...
    int32_t offset = layer_id * num_deploy_experts_ + expert_id;
    aclError ret = aclrtMemcpy(
        expert_activation_counts_ + offset,
        sizeof(int64_t),
        &value,
        sizeof(int64_t),
    ...
    if (ret != ACL_ERROR_NONE) {
        throw std::runtime_error("aclrtMemcpy failed in set_expert_activation_count at layer " +
                                 std::to_string(layer_id) + ", expert " +
                                 std::to_string(expert_id) + ", error code: " +
                                 std::to_string(ret));
    }
...
int64_t ClusterActivation::allGatherActivation(HcclComm comm, aclrtStream stream) {
    try {
        //执行集合通信操作
        int32_t ret = HcclAllGather(npu_count_.get_data_ptr(), (void*)expert_activation_counts_, num_layers_ * num_deploy_experts_per_rank_, HCCL_DATA_TYPE_INT64, comm, stream);
        if(ret != HCCL_SUCCESS)
        {
        ...
        throw std::runtime_error("Failed to reduce result to all rank: " + std::string(e.what()));
    }
}
