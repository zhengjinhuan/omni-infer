...
private:
    ...
    int64_t* expert_activation_counts_;    // activation all rank reduced
    ...
    void setExpertActivationCount(int32_t layer_id, int32_t expert_id, int64_t value);

public:
    ClusterActivation(Tensor npu_count,size_t num_layers, size_t num_deploy_experts_per_rank, int activation_window_size,size_t world_size, size_t rank);
    ~ClusterActivation();
    void collect_activation(size_t layer_idx, size_t deploy_expert_idx, int64_t count);
    void collect_activation(HcclComm comm, aclrtStream stream);
    int64_t allGatherActivation(HcclComm comm, aclrtStream stream);
    ...
    // 线程控制相关操作
    void collect_wrapper(HcclComm comm, aclrtStream stream);
    void start_thread();
    void stop_thread();
};
