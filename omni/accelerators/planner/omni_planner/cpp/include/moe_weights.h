...
class ExpertWeights {
    ...
    public:
        ...
        void enqueueSwapInformation(Distribution* dist_ptr, size_t layer_idx, SwapInstruction instruction);
...
public:
    ...
    void init_weights(const std::vector<std::vector<std::vector<Tensor>>>& npu_weights, const std::vector<std::vector<int>>& expert_ids,bool init_shm);

    void replacement(Distribution* dist_ptr, aclrtStream stream, size_t layer_idx, size_t local_expert_idx, size_t t_rank);

    void replacement(Distribution* dist_ptr, size_t layer_idx, SwapInstruction instruction);
