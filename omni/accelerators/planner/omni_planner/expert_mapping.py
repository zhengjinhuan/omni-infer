...
class ExpertMapping:
    def __init__(self, pattern_path, device: str = "npu", rank: int = 0, num_devices_per_host: int = 8):
        ...
        self._init_expert_intermediate_mapping()
    ...
    def _init_expert_intermediate_mapping(self) :
        self.intermediate_mapping = self.redundant_count_per_expert.clone()

    def get_intermediate_mapping(self) -> torch.tensor :
        return self.intermediate_mapping

    def update_working_mapping(self):
        intermediate_mapping = self.get_intermediate_mapping()

        if self.redundant_expert_mapping.shape != intermediate_mapping.shape:
            raise ValueError(
                f"Shape mismatch: redundant_expert_mapping.shape={self.redundant_expert_mapping.shape}, "
                f"intermediate_mapping.shape={intermediate_mapping.shape}"
            )

        if self.redundant_expert_mapping.dtype != intermediate_mapping.dtype:
            raise ValueError(
                f"Dtype mismatch: redundant_expert_mapping.dtype={self.redundant_expert_mapping.dtype}, "
                f"intermediate_mapping.dtype={intermediate_mapping.dtype}"
            )

        if self.redundant_expert_mapping.device != intermediate_mapping.device:
            raise ValueError(
                f"Device mismatch: redundant_expert_mapping.device={self.redundant_expert_mapping.device}, "
                f"intermediate_mapping.device={intermediate_mapping.device}"
            )

        self.redundant_expert_mapping.copy_(self.get_intermediate_mapping())
        print("Updated working mapping")
