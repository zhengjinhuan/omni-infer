...
class TestMoeWeightsWrapper(unittest.TestCase):
    ...
    def test_single_process_init_dram_weights(self):
        ...
        init_dram_weights(moeweights,self.param_dict,self.local_rank_pattern,self.first_k_dense_replace,init_shm=True)
    ...
    def test_zero_experts(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            init_dram_weights(moeweights, empty_param_dict, empty_pattern,self.first_k_dense_replace,init_shm=True)
    ...
    def test_zero_layers(self):
        ...
        moeweights = omni_placement.MoEWeights(self.num_expert, world_size)
        with self.assertRaises(RuntimeError) as context:
            init_dram_weights(moeweights, param_dict, local_rank_pattern,self.first_k_dense_replace,init_shm=True)
        self.assertFalse(moeweights.isShmInitialized())  # 确保未初始化
    ...
    def test_invalid_param_dict(self):
        ...
        with self.assertRaises(TypeError):  # 假设 init_dram_weights 检查类型
            init_dram_weights(moeweights, invalid_param_dict, self.local_rank_pattern,self.first_k_dense_replace,init_shm=True)
    ...
    def test_mismatched_local_rank_pattern(self):
        ...
        with self.assertRaises(IndexError):  # 假设 init_dram_weights 检查形状
            init_dram_weights(moeweights, self.param_dict, mismatched_pattern,first_k_dense_replace=self.first_k_dense_replace,init_shm=True)
