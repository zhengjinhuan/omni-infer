    ...
    def forward(self, x):
        if self.is_dynamic_quant:
            ...
            x, dynamic_scale = torch_npu.npu_dequant_swiglu_quant(
                ...
                activation_scale=dynamic_scale,  # activate_scale
                bias=None,
                quant_scale=None,
                quant_offset=None,
                group_index=None,
...
class DeepseekV3MoE(nn.Module):
    ...
    def __init__(
        ...
        DeepseekV3MoE.top_k = config.num_experts_per_tok
...
class DeepseekV3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if isinstance(
                self.mlp,
                DeepseekV3MLP) and hidden_states.dtype == torch.float16:
            # Fix FP16 overflow
...
class CustomDeepseekV3ForCausalLM(nn.Module):
