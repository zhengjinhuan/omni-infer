...
from omni.attention.attention import AscendAttentionState
from omni.ops.attention import vanilla_chunked_prefill_mla
from omni.attention.attention_mask import AttentionMaskBuilder
...
class AscendMLAMetadataBuilder:
    ...
    def __init__(self,
                 runner: "GPUModelRunner", kv_cache_spec: AttentionSpec = None,
                 block_table: BlockTable = None):
        self.metadata_cls: Optional[AscendMLAMetadata] =  AscendMLAMetadata  
        self.runner = runner
    ...
    def build(self,
        ...
        slot_mapping = self.runner.input_batch.block_table[
                0].slot_mapping_cpu[:num_actual_tokens].to(
    ...
    def build_dummy(self, num_tokens: int) -> AscendMLAMetadata:
        input_positions = torch.zeros(self.runner.scheduler_config.max_num_seqs,
                                  dtype=self.runner.positions_cpu.dtype,
                                  device=self.runner.device)
        slot_mapping = torch.zeros(self.runner.scheduler_config.max_num_seqs,
                                dtype=self.runner.slot_mapping_cpu.dtype,
                                device=self.runner.device)
        if isinstance(self.runner.graph_block_tables, np.ndarray):
            graph_block_tables = torch.zeros((self.runner.graph_block_tables.shape))
        
        block_table = graph_block_tables.to(
            device=self.runner.input_batch.block_table[0].get_device_tensor().device, 
            dtype=self.runner.input_batch.block_table[0].get_device_tensor().dtype)
        seq_lens = torch.ones(num_tokens, dtype=torch.int32, device="cpu", pin_memory=True)
        decode_metadata = AscendMLADecodeMetadata(
                input_positions=input_positions,
                block_table=block_table,
                seq_lens=seq_lens,
