...
def transform_config_for_playbook(logger, input_data):
    ...
    if 'max_model_len' not in input_data['services']['decode']:
        ...
        return None
    if 'your_local_code_path' not in input_data['services']:
        logger.error("Missing global configuration: %s", "your_local_code_path")
        return None
    ...
    updates["YOUR_LOCAL_CODE_PATH"] = input_data['services']['your_local_code_path']
...
def transform_deployment_config(config_path):
    ...
    try:
        ...
        update_yml_file(logger, playbookArges, f"{input_data['services']['your_local_code_path']}/omniinfer/omni_infer_server.yml")
