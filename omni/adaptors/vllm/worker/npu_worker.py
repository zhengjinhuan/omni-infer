...
class NPUWorker(WorkerBase):
    ...
    def _init_graph_options(self):
        ...
        self.block_num_floating_range = self.vllm_config.npu_compilation_config.block_num_floating_range
    ...
    def determine_available_memory(self) -> int:
        ...
        bk_range = self.block_num_floating_range * self.page_size_bytes()
        if self.use_cached_npu_graph:
            ...
            if check_torchair_cache_exists() and check_block_num_cache_exist():
                ...
                is_all_kv_cache_bytes_equal = all(torch.equal(t, old_kv_cache_bytes) for t in all_kv_cache_bytes)
                if not is_all_kv_cache_bytes_equal:
                        raise RuntimeError(f"The block num data of some ranks has been modified, origin: {old_kv_cache_bytes}")

                if abs(cur_npu_kv_cache_bytes - npu_kv_cache_bytes) > bk_range:
                        raise RuntimeError(f"The range between the current NPU kv_cache_bytes and the recorded kv_cache_bytes exceeds {bk_range} "
