...
from sglang.srt.layers.quantization.compressed_tensors.compressed_tensors_moe import CompressedTensorsMoEMethod
...
class AscendCompressedTensorsW8A8Int8MoEMethod(CompressedTensorsMoEMethod):
    ...
    def create_weights(
        ...
        w13_scale = torch.nn.Parameter(torch.empty(num_experts,
                                                  2 * intermediate_size_per_partition, 1,
                                                  dtype=torch.float32),
                                      requires_grad=False)
    ...
    def apply(
        ...
        raise NotImplementedError("MoE apply method should be implemented in subclass")