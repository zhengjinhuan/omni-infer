...
def add_node(args):
    """Add a node to omni_infer_deployment.yml under all.children.{role}.hosts."""
    # Locate default_profiles.yml and omni_infer_deployment.yml
    base_dir = os.path.dirname(__file__)
    default_path = os.path.join(base_dir, 'configs', 'default_profiles.yml')
    deploy_path = os.path.join(base_dir, 'configs', 'omni_infer_deployment.yml')
...
def rm_node(args):
    """Remove node from omni_infer_deployment.yml and reassign ports for the role."""
    base_dir = os.path.dirname(__file__)
    deploy_path = os.path.join(base_dir, 'configs', 'omni_infer_deployment.yml')
    deployment = load_yaml(deploy_path)
