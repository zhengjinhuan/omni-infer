...
#code modified from deepseek_v3_tool_parser.py
...
class StreamMatcher:
    def __init__(self, patterns):
        self.root = TrieNode()
        self.max_pat_len = max(len(p) for p in patterns)
        for pat in patterns:
            self._insert(pat)
        self.reset()
    ...
    def _feed(self, char):
        ...
        if char in self.root.children:
            new_active.append((self.root.children[char], char))
        for n, seq in self.active_nodes:
            if char in n.children:
                new_active.append((n.children[char], seq + char))
        self.active_nodes = new_active

        for n, seq in self.active_nodes:
            if n.is_end:
                unmatched_len = len(self.buffer) - len(seq)
                for c in self.buffer[:unmatched_len]:
                    outputs.append(('char', c))
                outputs.append(('match', n.pattern))
                self.reset()
                matched = True
                break
            
        if not matched and not self.active_nodes and self.buffer:
            outputs.append(('char', self.buffer[0]))
            self.buffer = self.buffer[1:]

        return outputs if outputs else None

@ToolParserManager.register_module(["ascend_kimi_k2"])
class AscendKimiK2ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.matcher = StreamMatcher([self.tool_call_start_token, self.tool_call_end_token,
                                      self.tool_calls_start_token, self.tool_calls_end_token, self.tool_call_argument_begin_token])
    ...
    def extract_tool_calls_streaming(
        ...
        current_matched_text = self.combine_char(matched)
        # check to see if we should be streaming a tool call - is there a
        if self.tool_calls_start_token not in current_text:
            logger.debug("No tool call tokens found!")
            return DeltaMessage(content=current_matched_text)
        delta_text = current_matched_text.replace(self.tool_calls_start_token,
